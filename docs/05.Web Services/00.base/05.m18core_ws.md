...
meta:
  - name: description
    content: Learning in Core Services for ERP system. Browse reference, sample code, tutorials, and more.
  - name: keywords
    content: Core Services, ERP, ERP software, ERP system, ERP solution
---
...
| footerOverwrite            | `boolean`(*Query*)    | Applies when a Footer detail table in the request is non-empty and **none** of its rows include an id. Default is true. When true, existing Footer rows are replaced with the submitted data. When false, submitted Footer rows are inserted as new rows and existing Footer data is kept. |
| footerPrune                | `boolean`(*Query*)    | Applies when a Footer detail table in the request is non-empty and **at least one** row includes an id. Default is false. When false, existing Footer rows whose id is not present in the submission are kept. When true, those unmatched rows are deleted. Regardless of `footerPrune`, rows without an id are appended, and rows with a matching id are updated. |
...
When a Footer detail table is **non-empty** and **at least one row includes id**, use the query parameter `footerPrune` to control whether existing Footer rows that do not appear in this submission are removed (see the parameters table above).
...
Example entity JSON for the request body:
...
State of the Footer before save:
...
When `footerPrune` is false (the default), after save the Footer looks like this:
...
When `footerPrune` is true, after save the Footer looks like this:
...
### Fetch Print File by Type

Same as [Fetch PDF File](#fetch-pdf-file), except the export format is specified by path parameter `fileType`. Use this API when you need formats other than PDF.
...
| authorization | `string`(*Header*) | **Required.** Access Token obtained via Oauth2               |
| client_id     | `string`(*Header*) | **Required.** When registered in [OAuth Applications], generated by the aiM18 |
| fileType      | `string`(*Path*)   | **Required.** Export format. Supported values (lowercase): `pdf`, `xlsx`, `html`, `rtf`. |
| menuCode      | `string`(*Query*)  | **Required.** Menu code, such as 'employee', can be found in data dictionary. |
| recId         | `long`(*Query*)    | **Required.** ID of the entity.                              |
| printOptionId | `long`(*Query*)    | ID of Print Options.                                         |
...
| authorization | `string`(*Header*) | **Required.** Access Token obtained via Oauth2              |
| client_id     | `string`(*Header*) | **Required.** When registered in [OAuth Applications], generated by the aiM18 |
| imgKey        | `string`(*Path*)   | **Required.** Image code |
| thumbnail     | `boolean`(*Query*) | If true, return the thumbnail. |

 

#### Result

> If API runs success, status of the response will be 200, then it will return the Image File.
> If API runs failed , status of the response will be 400, then it will return the CheckMsg. You can get a CheckMsg JSON array from the response header with key word 'error_info'.
...
| authorization | `string`(*Header*) | **Required.** Access Token obtained via Oauth2               |
| client_id     | `string`(*Header*) | **Required.** When registered in [OAuth Applications], generated by the aiM18 |
| moduleType    | `string`(*Query*)  | **Required.** Module type, such as 'employee', can be found in [Data Dictionary]. |
| menuCode      | `string`(*Query*)  | **Required.** Menu code of the editor, can be found in [Data Dictionary].         |
| table     	| `string`(*Query*)  | **Required.** Table Name of the module, can be found in [Data Dictionary].        |
| app     		| `string`(*Query*)  | App Name, can be found in [Data Dictionary].                 |
...
| module                     | `String`(*Path*)      | **Required.** Module type,such as 'employee', can be found in data dictionary. |
| menuCode                   | `String`(*Query*)     | **Required.** Menu code, such as 'employee', can be found in data dictionary.  |
| recordId                   | `long`(*Query*)       | **Required.** The id of the entity                                             |
| entity                     | `jsonString`(*Query*) | The Simplified Entity JSON, structure please refer to the below. Notice that the JSON need to be <u>URL Encoded</u>. |
| entitys_in_entity          | `String`(Body)        | The Simplified Entity JSON, structure please refer to the below. It is recommended to pass entity JSON in request body.If you want to handle more entitys, please pass an array of Simplified Entity JSON. |
| autoSubmit                 | `boolean`(*Query*)    | If the value is true, the system will submit the record automatically for approval. |
| changenoteCode             | `string`(*Query*)     | Change note code. When empty, create a new change note; when not empty, update the corresponding change note. |
| remark                     | `string`(*Query*)     | Change note remark                                                             |
...
> If API runs success, status of the response will be 200, then it will return the code and ID of the saved change note. If API run failed , status of the response will be 400, then it will return the CheckMsg. You can get a CheckMsg JSON array from the response header with key word 'error_info'.

| Type             | Location              | Description                             |
| ---------------- | --------------------- | --------------------------------------- |
| `success`        | *Body*                | code and ID of the saved change note    |
| `fail`           | *Header*(error_id=1)  | Multiple errors with JSON array format  |
| `fail`           | *Header*(error_info)  | A CheckMsg JSON array                   |
